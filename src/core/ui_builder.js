import { loadResources } from '../js/CytubeEnhancedHelpers/loadResources.js';

export class ThemeLayout {

    static async buildMobileLayout() {
    ...
    static async buildDesktopLayout() {
        try {
            await loadResources([
                { type: 'css', href: `${process.env.CDN_URL}${process.env.DEV_BRANCH}/src/css/base.css` },
                { type: 'css', href: `${process.env.CDN_URL}${process.env.DEV_BRANCH}/src/css/city_theme.css` },
                { type: 'js', src: `${process.env.CDN_URL}${process.env.DEV_BRANCH}/src/assets/notifications.js` },
                { type: 'js', src: `${process.env.CDN_URL}${process.env.DEV_BRANCH}/src/assets/avatars.js` },
            ]);
        } catch (err) {
            console.error('Failed to load resource:', err.message, err);
        }
